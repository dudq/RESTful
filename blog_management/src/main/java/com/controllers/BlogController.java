...
public class BlogController {
    ...
    public ResponseEntity<Void> createBlog(@RequestBody Blog blog, UriComponentsBuilder componentsBuilder) {
        System.out.println("Creating Blog " + blog.getTitle());
        blogService.save(blog);
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.setLocation(componentsBuilder.path("/blogs/{id}").buildAndExpand(blog.getId()).toUri());
    ...
    public ResponseEntity<Blog> deleteBlog(@PathVariable Long id) {
        ...
        if (blog == null) {
            System.out.println("Unable delete blog with id " + id + " not found");
            return new ResponseEntity<Blog>(HttpStatus.NOT_FOUND);
        }
